...
def build_attn_pair_bias(size, dtype, device, *, H=4, D=32, Cz=128, **kw):
    ...
    W_pz = _rnd(Cz, H, dtype=dtype, device=device, sd=Cz**-0.5, seed=6)
    B_pz = torch.zeros(H, device=device, dtype=dtype)
    W_pg = _rnd(N, N, dtype=dtype, device=device, sd=N**-0.5, seed=7)
    B_pg = torch.zeros(N, device=device, dtype=dtype)
    W_po = _rnd(N, N, dtype=dtype, device=device, sd=N**-0.5, seed=8)
...
def build_triangle_attn(size, dtype, device, *, H=4, D=32, Cin=128, **kw):
    ...
    W_pz = _rnd(Cin, H, dtype=dtype, device=device, sd=Cin**-0.5, seed=6)
    B_pz = torch.zeros(H, device=device, dtype=dtype)
