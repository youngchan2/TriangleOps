...
def precompute(norm_in_weight, norm_in_bias, p_in_weight, g_in_weight,
    ...
    return {
        "p_in_combined": p_in_c, "g_in_combined": g_in_c,
        "p_out_combined": p_out_c, "g_out_combined": g_out_c,
        "sum_W_p_in": sWp_in, "sum_W_g_in": sWg_in,
        "sum_W_p_out": sWp_out, "sum_W_g_out": sWg_out,
        "B_p_in_const": Bp_in, "B_g_in_const": Bg_in,
        "B_p_out_const": Bp_out, "B_g_out_const": Bg_out,
    }
...
def forward(x, pre, *, direction, mask=None, eps=1e-5):
    ...
    ab_t = input_phase(
        x, mask,
        pre["p_in_combined"], pre["g_in_combined"],
        pre["sum_W_p_in"], pre["sum_W_g_in"],
        pre["B_p_in_const"], pre["B_g_in_const"], eps,
    )                                              # (2D, B, L, L)
    ...
    return output_phase(
        y, x,
        pre["p_out_combined"], pre["g_out_combined"],
        pre["sum_W_p_out"], pre["sum_W_g_out"],
        pre["B_p_out_const"], pre["B_g_out_const"], eps,
    )


def triangle_multiplicative_update(
    x, *, direction, mask=None, eps=1e-5,
    ...
    pre = precompute(norm_in_weight, norm_in_bias, p_in_weight, g_in_weight,
                     norm_out_weight, norm_out_bias, p_out_weight, g_out_weight)
